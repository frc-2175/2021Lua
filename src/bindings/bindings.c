...
int main(int argc, char** argv) {
    ...
    MD_String8List luaImports = {0};

    for (MD_EachNode(f, parse.node->first_child)) {
        fprintf(stderr, "Processing file \"%.*s\"...\n", MD_StringExpand(f->string));
        MD_PushStringToList(&luaImports, MD_PushStringF("require(\"wpilib.bindings.%.*s\")", MD_StringExpand(f->string)));
        ...
        sprintf(filename_buf, "src/lua/wpilib/bindings/%.*s.lua", MD_StringExpand(f->string));
    ...
    // Output Lua imports file
    FILE* luaimportsfile = fopen("src/lua/wpilib/bindings/init.lua", "w");
    fprintf(luaimportsfile, "%.*s\n", MD_StringExpand(MD_JoinStringList(luaImports, MD_S8Lit("\n"))));
    fclose(luaimportsfile);
